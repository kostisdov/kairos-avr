...
def test_form_loads_the_first_example(app):
    ...
    assert app.title[0].value == "KAIROS: patient model"
...
def test_reliability_reasons_are_rendered(app):
    app.button(key="run_predict").click().run()
    assert not _errors(app)
    assert any(m.value == "**Reasons**" for m in app.markdown)
    ...
    for reason in body["reliability"].get("reasons", []):
        assert f"`{reason['code']}`" in shown and reason["message"] in shown
    if not body["reliability"].get("reasons"):
        assert any("No reliability reasons reported" in c.value for c in app.caption)


def test_comparison_tab_reruns_on_edit(app):
    assert not _errors(app)
    results = app.session_state["res_compare"]["results"]
    assert app.selectbox(key="model_family").value in results
    assert len(results) == len(app.selectbox(key="model_family").options)
    assert any("SVD before death, 12 months (" in m.label and "model " in m.label for m in app.metric)
    key_before = app.session_state["res_compare"]["key"]
    app.toggle(key="p_dialysis").set_value(True).run()
    assert not _errors(app)
    assert app.session_state["res_compare"]["key"] != key_before
